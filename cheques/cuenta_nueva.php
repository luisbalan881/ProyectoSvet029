...
if (function_exists('login_check') && login_check()):
    if (usuarioPrivilegiado()->hasPrivilege('crearCheques')):
        $bancos = array();
        if ( !empty($_POST)) {
            Account::create(usuarioPrivilegiado(),$_POST['cta_titular'],$_POST['cta_num'],$_POST['bc_id'],$_SESSION['user_id']);
            ...
            <div class="block-content">
                <form class="js-validation-banco form-horizontal push-10-t push-10" action="<?php echo htmlentities($_SERVER['PHP_SELF']); ?>" method="POST">
                            ...
                            <div class="form-material">
                                <select class ="js-select2 form-control" name="bc_id" id="bc_id"  style="width: 100%;" data-placeholder="-- Seleccionar Banco --" required>
                                    ...
                                    foreach ($bancos as $banco):
                                            echo '<option value="'.$banco["bc_id"].'" '.(($banco["bc_status"] == 0)? 'disabled':'').'>'.$banco["bc_nombre"].'</option>';
                                    endforeach
                                    ?>
                                </select>
                    ...
                    <div class="form-group">
                        <div class="col-xs-12">
                          <div class="form-material">
                              <input class="form-control focus"  type="text"  id="cta_titular" name="cta_titular"  required>
                    ...
                    <div class="form-group">
                        <div class="col-xs-12">
                            <div class="form-material">
                                <input class="form-control focus"  type="text"  id="cta_num" name="cta_num"  required>
                    ...
                    <div class="form-group">
                      <div class="col-xs-12 text-center">
                          <button class="btn btn-sm btn-success btn-block" type="submit"><i class=""></i>Crear Cuenta</button>
                      </div>
                    </div>
                </form>
            </div>
