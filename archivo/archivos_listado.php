          ...
          <div class="row items-push">
              ...
              <div class="col-sm-5 text-right hidden-xs">
                  <ol class="breadcrumb push-10-t">
                      ...
                      <li><a class="link-effect" href="#">Archivos Enviados</a></li>
                  </ol>
              </div>
          </div>
        </div>
        ...
        <div class="content content-boxed">
                ...
                <?php if ($u->hasPrivilege('crearArchivo')): ?>
                    ...
                    <div class="block block-themed block-rounded" id="block_hide">
                        <div class="block-header bg-success">
                            ...
                            <span id="block_show" class="text-white"><h3 class="block-title">+ Agregar Nuevo Archivo Enviado</h3></span>
                        </div>
                        <div class="block-content block-content-full">
                            <form class="js-validation-documento form-horizontal push-10-t push-10" action="archivo/archivo_nuevo.php" method="POST" enctype="multipart/form-data">
                                <div class="form-group">
                                    <div class="col-xs-12">
                                        <div class="form-material">
                                          <div class="input-group has-personalizado">
                                            <span class="input-group-addon" ><span class="fa fa-bank"></span></span>
                                            <select name="a_destinatarios[]" multiple="multiple" data-placeholder="Seleccione una o mas instituciones" class="chosen-select col-xs-12" multiple tabindex="6" required>
                                              <?php
                                ...
                                <div class="form-group">
                                    <div class="col-xs-12">
                                        <div class="form-material">
                                          <div class="input-group has-personalizado">
                                            <span class="input-group-addon" ><span class="fa fa-bank"></span></span>
                                            <select name="a_destinatarios_c[]" multiple="multiple" data-placeholder="Seleccione una o mas instituciones" class="chosen-select col-xs-12" multiple tabindex="6">
                                              <?php
                                              foreach ($instituciones as $tipo):
                                                  if ($tipo['inst_status'] == 1){
                                                      echo '<option value="'.$tipo["inst_id"].'">'.$tipo["inst_nombre"].'</option>';
                                    ...
                                    <div class="col-xs-4">
                                        <div class="form-material">
                                          <div class="input-group has-personalizado">
                                            ...
                                            <select class ="chosen-select" name="tipo_id" id="tipo_id"  style="width: 100%;" data-placeholder="-- Seleccionar Tipo --" required>
                                                <option></option>
                                                <?php
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="col-xs-4">
                                        <div class="">
                                            <input class=""  type="file" id="arch_firmado" name="arch_firmado" accept="application/pdf">
                                            <label for="arch_firmado">Archivo Firmado</label>
                                        </div>
                                    </div>
                                    <div class="col-xs-4">
                                        <div class="">
                                            <input class=""  type="file" id="arch_recibido" name="arch_recibido" accept="application/pdf">
                                ...
                                <div class="form-group">
                                    <div class="col-xs-12 text-center">
                                        <button class="btn btn-sm btn-success btn-block" type="submit"><i class=""></i>Agregar Archivo</button>
                                    </div>
                                </div>
                            </form>
                        </div>
            ...
            <div class="block block-themed block-rounded">
              <div class="block-header  bg-muted">
                  ...
                  <h3 class="block-title text-white">Listado de Archivos Enviados</h3>
              </div>
              <div class="block-content">
                      <table id="archivos-recibidos-table" class="table table-bordered table-condensed table-striped  js-dataTable-documentos dt-responsive display nowrap" cellspacing="0" width="100%">
                          <thead>
                          <tr>
                          ...
                          foreach ($archivos as $archivo){
                              $dir_original = sanear_string($archivo['depto_nombre']).'/'.$archivo['arch_original'];
                              $dir_firmado = sanear_string($archivo['depto_nombre']).'/'.$archivo['arch_firmado'];
                              $dir_recibido = sanear_string($archivo['depto_nombre']).'/'.$archivo['arch_recibido'];
                              echo '<tr>';
                              echo '<td class="text-center" style="white-space: nowrap;">'.fecha_dmy($archivo['arch_fecha']).'</td>';
                              $destinatarios = explode(';',$archivo['arch_destinatario']);
                              $destinatarios_cc = explode(';', $archivo['arch_destinatario_cc']);
                              $destinatariosTotal = count($destinatarios);
                              $destinatariosCCTotal = count($destinatarios_cc);
                              echo '<td class="text-left">';
                              echo (($destinatariosTotal > 1)?'<ul style="padding:0">':'');
                              foreach ($destinatarios as $destinatario) {
                                  if ($destinatario != '') {
                                      echo (($destinatariosTotal > 1)?'<li style="list-style:none"><br>':'');
                                      echo $destinatario;
                                      echo (($destinatariosTotal > 1)?'</li>':'');
                                  }
                              }
                              echo (($destinatariosTotal > 1)?'</ul>':'');
                              echo '</td>';
                              echo '<td class="text-left">';
                              echo (($destinatariosCCTotal > 1)?'<ul>':'');
                              foreach ($destinatarios_cc as $destinatario) {
                                  if ($destinatario != '') {
                                      echo (($destinatariosTotal > 1)?'<li>':'');
                                      echo $destinatario;
                                      echo (($destinatariosTotal > 1)?'</li>':'');
                              ...
                              echo '</ul>';
                              echo '</td>';
                              echo '<td class="text-left">'.$archivo['tipo_nombre'].'</td>';
                              echo '<td class="text-left">'.$archivo['arch_correlativo'].'</td>';
                              echo '<td class="text-left">'.$archivo['arch_titulo'].'</td>';
                              echo '<td class="text-left">';
                              echo '<span style="white-space: nowrap;"><strong>Original: </strong><a target="_blank" href="archivo/adjuntos/'.$dir_original.'"><button class="btn btn-xs btn-default outline" type="button" data-toggle="tooltip" title="Ver Original"><i class="fa fa-file"></i> Ver Original</button></a></span> <br>';
                              echo '<span style="white-space: nowrap;"><strong>Firmado:  </strong>'.(($archivo['arch_firmado'] != '')?'<a target="_blank" href="archivo/adjuntos/'.$dir_firmado.'"><button class="btn btn-xs btn-default outline" type="button" data-toggle="tooltip" title="Ver Firmado"><i class="fa fa-file "></i> Ver Firmado</button></a>':' No Disponible').'</span> <br>';
                              echo '<span style="white-space: nowrap;"><strong>Recibido: </strong>'.(($archivo['arch_recibido'] != '')?'<a target="_blank" href="archivo/adjuntos/'.$dir_recibido.'"><button class="btn btn-xs btn-default outline" type="button" data-toggle="tooltip" title="Ver Recibido"><i class="fa fa-file "></i> Ver Recibido</button></a>':' No Disponible').'</span> ';
                              echo '</td>';
                              echo '<td class="text-center">'.$archivo['depto_nombre'].'</td>';
                              echo '<td class="text-left" style="white-space: nowrap;">'.$archivo['arch_user_nombre'].'</td>';
                              echo '<td class="text-left">';
                              ...
                              if($u->hasPrivilege('modificarArchivo')) {
                                  ...
                                  if ($archivo['user_id'] == $user->persona['user_id'] || $archivo['dep_encargado'] == $user->persona['user_id']){
                                      echo '<span  title="Editar"><a class="btn btn-default outline" data-toggle="modal" data-target="#modal-remoto-lg" href="archivo/archivo_modificar.php?id=' . $archivo['arch_id'] . '"><i class="fa fa-pencil "></i></a></span>';
                                      echo ' ';
                                      echo '<span  title="Eliminar"><a class="btn btn-default outline" data-toggle="modal" data-target="#modal-remoto-lg" href="archivo/archivo_eliminar.php?id=' . $archivo['arch_id'] . '"><i class="fa fa-trash "></i></a></span>';
                                  }
                                  echo '</div>';
                                  echo '</td>';
                              }
