                  function HTMLtoPDFV111(solicitud){
	$.ajax({
    type: "POST",
    url: "viaticos/php/hoja_solicitud22.php",
...
//destino
	...
	var doc = new jsPDF();
        ...
        var logo1 = new Image();
        logo1.src = 'assets/img/anticipo2.jpeg';
	   doc.addImage(logo1, 'jpeg', 15, 6, 303, 265);
	   ...
	   //doc.text(33, 50, 'BOLETA DE SOLICITUD Y AUTORIZACIÓN DE VEH�?CULO PARA COMISIONES');
		...
		// doc.writeText(0, 65 ,'SOLICITUD DE VIATICOS 6',{align:'center',width:211});
		 doc.setFontSize("10");
                  doc.text(170, 39, data.id1);
                   // doc.text(175, 45, data.Total);
         //   var p_lineas2 = doc.splitTextToSize(data.obj, 73);

		// doc.text(22, 86, p_lineas2); 
		// doc.text(25, 80, 'Nombramiento :');
		 //doc.text(25, 86, 'Autorizado :');
                 
                  //doc.text(97, 86, data.lugar1);
                  // var p_lineas22 = doc.splitTextToSize(data.lugar1, 59);

		 //doc.text(96, 86, p_lineas22); 
                  
                  
                  
                 //  doc.text(170, 86, data.Total2);
		...
		// var p_lineas1 = doc.splitTextToSize(data.departamentos, 115);
            //    doc.text(86, 118,  data.correlativo);
              //  doc.text(173, 118,  data.fecha1);
		// doc.text(55, 92, p_lineas1);
		// doc.text(40, 147, data.autorizado2);
                 //doc.text(40, 157, data.puesto);
                  
                 //  doc.text(40, 193, data.autorizado);
                   // doc.text(40, 203, data.puesto2);
                    // doc.writeText(0, 222 ,'Guatemala, guatemala '+data.fechaf,{align:'center',width:190});
