...
//destino
	...
	var doc = new jsPDF();
	   ...
	   //doc.text(33, 50, 'BOLETAs DE SOLICITUD Y AUTORIZACIÓN DE VEH�?CULO PARA COMISIONES');

		 doc.setFontSize(12.5);
		// doc.writeText(0, 65 ,'SOLICITUD DE VIATICOS 6',{align:'center',width:211});
		 doc.setFontSize("10");
                  doc.text(180, 37, data.id1);
                    doc.text(175, 44, data.Total);
                    
                    
                     doc.writeText(40, 59 ,'SECRETARIA CONTRA LA VIOLENCIA SEXUAL EXPLOTACION Y TRATA DE PERSONAS.');
		...
		// doc.text(25, 80, 'Nombramiento :');
		 //doc.text(25, 86, 'Autorizado :');
                  ...
                  //doc.text(69, 79, data.lugar1);
                   var p_lineas5 = doc.splitTextToSize(data.lugar1, 40);
		 ...
		 doc.text(76, 94, p_lineas5); 
                   ...
                   doc.text(114, 94, data.Total2);
				//   	doc.writeText(144, 94 ,'250.00');
				doc.writeText(144, 94 ,'420.00');
				   // doc.text(144, 95, data.Total2);
		...
		// var p_lineas1 = doc.splitTextToSize(data.departamentos, 115);
              //  doc.text(80, 215,  data.correlativo);
			   ...
			   doc.text(173, 97,  data.tgas);
                //doc.text(173, 103,  data.almu);
                //doc.text(173, 109,  data.cena);
                //doc.text(173, 115,  data.hosp);//tgas
               // doc.text(173, 126,  data.tgas);
             //   doc.text(173, 128,  data.tgas);
                doc.text(173, 140, data.Totalsolicitado);  //total solicitado normalmente sin ampliacion
				  
				//  var t1= data.tgas;
				//  var t3= data.Totalsolicitado;
			/*	doc.writeText(173, 147 ,'00.00');
                   doc.text(173, 154,  data.complemento); //complemento*/
				   
				    if (data.complemento>0){
						 
						 doc.text(173, 157,  data.complemento);///	��������
						  doc.writeText(173, 152 ,'00.00'); // reitegro a la dep
                     }
					 else
						  doc.text(173, 147,  data.complemento);////
				   doc.writeText(173, 154 ,'00.00');
		...
		// doc.text(55, 92, p_lineas1);
		 ...
		 doc.text(31, 185, data.autorizado2);
				...
				//  doc.writeText(161, 218 ,'NIT:');
                   doc.text(161, 216, data.nit);
                 //  doc.writeText(128, 192 ,'Q');
                //   doc.text(131, 192, data.sueldo); //sueldo
               //    doc.text(89, 198, data.partida); // partida presupuestaria
                  
                   doc.text(139, 232, data.autorizado);
                    doc.text(127, 238, data.puesto2);
